...
class VortexEngine:
    ...
    def __init__(self) -> None:
        ...
        self.objects = [
            FlowObject("Object 1", "Torus", "Plasma", np.array([0.0, 0.0, -2.0]),
                       {"radius": 1.5, "thickness": 0.35, "density": 1.0, "circulation": 5.0, "magnetic": 1.8},
                       (0.12, 0.65, 0.95, 0.85)),
            FlowObject("Object 2", "Torus", "Plasma", np.array([0.0, 0.0, 2.0]),
                       {"radius": 1.55, "thickness": 0.38, "density": 1.1, "circulation": 5.2, "magnetic": 2.1},
                       (0.96, 0.52, 0.25, 0.8)),
        ]
        self.time = 0.0
...
class VortexMath6App:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Vortex Math 6.0")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(header, text="VORTEX MATH 6.0", font=("Segoe UI", 18, "bold")).pack(anchor="w")
    ...
    def _render(self) -> None:
        ...
        self.ax.set_title(f"Vortex Math 6.0 | t = {self.engine.time:.2f} s | {self.engine.environment.magnetic:.2f} T", color=theme[3], pad=12)
        self.ax.set_xlabel("x [m]"); self.ax.set_ylabel("y [m]"); self.ax.set_zlabel("z [m]")
        values = self.engine.metrics(fields); self.lesson.configure(text=f"Density {values['density']:.3e}   Kinetic {values['kinetic']:.3e}   Magnetic energy {values['magnetic_energy']:.3e}   Magnetic pressure {values['magnetic_pressure']:.3e} Pa")
    ...
    def _export_metrics(self) -> None:
        ...
        if path:
            ...
            with open(path, "w", newline="", encoding="utf-8") as handle: csv.DictWriter(handle, fieldnames=values.keys()).writeheader(); csv.DictWriter(handle, fieldnames=values.keys()).writerow(values)

    def _save_scene(self) -> None:
        path = filedialog.asksaveasfilename(defaultextension=".json", filetypes=(("JSON scene", "*.json"),))
        if path:
            data = {"version": "6.0", "environment": asdict(self.engine.environment), "spacetime": asdict(self.engine.spacetime), "time": self.engine.time, "objects": [{"label": o.label, "shape": o.shape, "phase": o.phase, "position": o.position.tolist(), "params": o.params} for o in self.engine.objects]}
