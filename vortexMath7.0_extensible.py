#!/usr/bin/env python3
"""Vortex Math 6.0: a standalone analytic field laboratory.

This version owns both the simulation engine and the Tkinter/Matplotlib GUI.
It does not import vortexMath4.0.py or any other Vortex Math application.
The equations are intentionally lightweight teaching models, not a full CFD or
MHD solver.
"""
    ...
    def run(self, engine: "VortexEngine") -> ExperimentResult:
        ...
        try:
            for value in values:
                ...
                fields = engine.fields(engine.grid())
...
class PluginRegistry:
    """Central registry for bolt-on capabilities."""
...
class VortexMath6App:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Vortex Math 7.0 — Extensible Field Laboratory")
    ...
    def _build_ui(self) -> None:
        ...
        self.object_tab, self.env_tab, self.time_tab, self.visual_tab = [ttk.Frame(self.notebook, padding=10) for _ in range(4)]
        for tab, name in ((self.object_tab, "Objects"), (self.env_tab, "Environment"), (self.time_tab, "Time"), (self.visual_tab, "Visuals")):
            self.notebook.add(tab, text=name)
        self._build_objects(); self._build_environment(); self._build_time(); self._build_visuals()
    ...
    def _build_visuals(self) -> None:
        ...
        ttk.Button(section, text="Reset simulator", command=self._reset_simulator).pack(fill="x", pady=(10, 2))
    ...
    def _sync_controls(self) -> None:
        ...
        spacetime = self.engine.spacetime
        slider_values = {
            "pressure": environment.pressure,
            "temperature": environment.temperature,
            "magnetic": environment.magnetic,
            "gravity": environment.gravity,
            "viscosity": environment.viscosity,
            "container_size": environment.container_size,
            "time_scale": spacetime.time_scale,
            "time_warp": spacetime.time_warp,
            "time_dilation": spacetime.time_dilation,
        }
        for index, obj in enumerate(self.engine.objects):
            self.vars[f"shape{index}"].set(obj.shape)
            self.vars[f"phase{index}"].set(obj.phase)
            for key, value in obj.params.items():
                scale = self.vars.get(f"scale_{key}{index}")
                if isinstance(scale, tk.Scale):
                    scale.set(value)
        for key, value in slider_values.items():
            scale = self.vars.get(f"scale_{key}")
            if isinstance(scale, tk.Scale):
                scale.set(value)
        self.container.set(environment.shape)
        self.boundary.set(environment.boundary)

    def _reset_simulator(self) -> None:
        """Restore the initial engine, controls, view options, and clock."""
        self.running = False
        ...
        self.field_lines.set(True)
        self.curvature.set(True)
        self._sync_controls()
        self._apply_theme()
    ...
    def _render(self) -> None:
        ...
        fields = self.engine.fields(self._grid()); mask = fields["density"] > .05
        if np.any(mask):
            x, y, z = self._grid()
            scatter: Any = self.ax.scatter
            scatter(x[mask], y[mask], z[mask], c=fields["density"][mask], cmap="inferno", s=4, alpha=.25)
        self.ax.set_title(f"Vortex Math 7.0 | t = {self.engine.time:.2f} s | {self.engine.environment.magnetic:.2f} T", color=theme[3], pad=12)
    ...
    def _animation_tick(self) -> None:
        if self.running: self.engine.step(); self._queue_render()
    ...
    def _save_scene(self) -> None:
        ...
        if path:
            data = {"version": "7.0", "format": "vortex-scene", "environment": asdict(self.engine.environment), "spacetime": asdict(self.engine.spacetime), "time": self.engine.time, "objects": [{"label": o.label, "shape": o.shape, "phase": o.phase, "position": o.position.tolist(), "params": o.params} for o in self.engine.objects]}
